...
def test_validate_then_propagate_epistemic():
    ...
    # Add two nodes: A (high confidence), B (medium)
    ev_a = add_node.execute(graph, "Rain", NodeType.FACT, Confidence(0.9))
    a_id = ev_a.node_id
    ev_b = add_node.execute(graph, "Wet road", NodeType.CONCEPT, Confidence(0.5))
    b_id = ev_b.node_id
    ...
    assert result.is_valid or len(result.violations) == 0  # no blocking violations

    # Confidence of B before propagation
    node_b_before = graph.get_node(b_id)
    assert node_b_before is not None
    conf_before = float(node_b_before.confidence)

    # Propagate from A (epistemic: SUPPORTS strengthens B)
    ...
    assert conf_after >= conf_before  # B's confidence should increase or stay same
    assert conf_after > 0.5  # should have been strengthened